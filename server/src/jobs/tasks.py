...
# ---------------------------------------------------------------------------
# Async Job 1 — Student Applications CSV Export
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Async Job 2 — Company Applications CSV Export
# ---------------------------------------------------------------------------
...
@celery.task(bind=True)
def export_company_applications_csv(self, company_id, export_job_id):
    """Generate a CSV of all applications across a company's drives."""
    logger.info(
        "Starting company CSV export: company=%d job=%d",
        company_id,
        export_job_id,
    )
    try:
        ...
        exports_dir = _resolve_path("static", "uploads", "exports")
        os.makedirs(exports_dir, exist_ok=True)
        filename = f"company_{company_id}_{export_job_id}.csv"
        filepath = os.path.join(exports_dir, filename)
        ...
        for app_rec in applications:
            ...
            rows.append(
                [
                    ...
                    _na(drive.job_location),
                    student.full_name,
                    _na(student.user.email.split("@")[0]),
                    student.branch,
                    student.cgpa,
                    student.year,
                    app_rec.status,
                    ...
                    _na(app_rec.interview_mode),
                    _na(app_rec.company_remarks),
                    "Yes" if student.is_placed else "No",
                ]
            )

        _write_csv(filepath, COMPANY_CSV_COLUMNS, rows)
        ...
        notification = Notification(
            ...
            title="Export Ready",
            message=("Your applications export CSV is ready for download."),
        )
        db.session.add(notification)
        db.session.commit()

        logger.info(
            "Company CSV export done: %s (%d rows)",
            filepath,
            len(applications),
        )
    except Exception:
        logger.exception(
            "Company CSV export failed: job=%d",
            export_job_id,
        )
        try:
            job = db.session.get(ExportJob, export_job_id)
